...
movies = os.listdir(scripts_path)
...
for movies in movies:
    ...
    """ 등장인물 별 대사 량을 측정 현재로썬 필요없어보임
    txt_len_role = {}
    for role, txt in scripts_by_role.items():
        txt_len_role[role] = len(scripts_by_role[role])
    sorted_dict = sorted(txt_len_role.items(), key = lambda item: item[1], reverse = True)[:10]
    """
